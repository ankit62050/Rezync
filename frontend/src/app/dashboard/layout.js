...
// Create a context to share the backend-synced unique username
export const DashboardContext = createContext({ username: 'candidate', loading: true });
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Fallback to clerk values
        if (user) {
          setUsername(user.username || user.firstName || 'candidate');
        }
      } finally {
  ...
  return (
    <DashboardContext.Provider value={{ username, loading: dbUserLoading }}>
